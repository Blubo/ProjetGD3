...
public class Rope2DLouis
{
	...
	//"Distance"
	//"Vector2" ?
	//"tempChainHinge.enabled"
	//"hingeHolder[i].enabled"
...
//				chain.collider2D.isTrigger = true;
				chain.GetComponent<Collider>().isTrigger = true;
...
//					DistanceJoint2D	distJoint = new DistanceJoint2D();
					...
					var joints = pointA.GetComponentsInChildren<SpringJoint>(); //get all SpringJoint component from pointA
					SpringJoint	distJoint = new SpringJoint();
...
//					distJoint.distance = 0.01f;
					distJoint.maxDistance = 0.01f;
//					distJoint.connectedBody = chain.rigidbody2D;
//					pointA.rigidbody2D.isKinematic = true;
					distJoint.connectedBody = chain.GetComponent<Rigidbody>();
					pointA.GetComponent<Rigidbody>().isKinematic = true;
				}
//				else chain.rigidbody2D.isKinematic = lockFirstChain;
				else chain.GetComponent<Rigidbody>().isKinematic = lockFirstChain;
			}
...
//				var tempChainHinge = tempChain.GetComponent<HingeJoint2D>();
				var tempChainHinge = tempChain.GetComponent<HingeJoint>();
...
//				tempChainHinge.connectedBody = chain.rigidbody2D;
				tempChainHinge.connectedBody = chain.GetComponent<Rigidbody>();
...
//					chainHinge.connectedBody = tempChain.rigidbody2D;
					var chainHinge = chain.gameObject.AddComponent<HingeJoint>();
					chainHinge.connectedBody = tempChain.GetComponent<Rigidbody>();
					//set anchor position
					if(!connectToA)
					{
...
//				if(!tempChain.GetComponent<HingeJoint2D>())
				if(!tempChain.GetComponent<HingeJoint>())
				{
...
//				var tempChainJoint = tempChain.GetComponent<HingeJoint2D>();
				var tempChainJoint = tempChain.GetComponent<HingeJoint>();
...
//					DistanceJoint2D	distJoint = new DistanceJoint2D();
					var joints = pointB.GetComponentsInChildren<SpringJoint>(); //get all DistanceJoint2D from pointB
					SpringJoint	distJoint = new SpringJoint();
...
//					pointB.rigidbody2D.isKinematic = true;
					distJoint.connectedBody = chain.GetComponent<Rigidbody>();
					pointB.GetComponent<Rigidbody>().isKinematic = true;
				}
				else if(lockLastChain) 
//					chain.rigidbody2D.isKinematic = true;
					chain.GetComponent<Rigidbody>().isKinematic = true;


//				GameObject.DestroyImmediate (chain.GetComponent<HingeJoint2D>()); //remove HingeJoint2D component for last chain
				GameObject.DestroyImmediate (chain.GetComponent<HingeJoint>()); //remove HingeJoint component for last chain
...
//					var joints = pointA.GetComponentsInChildren<DistanceJoint2D>();	//get all DistanceJoint2D component from pointA
//					DistanceJoint2D	distJoint = new DistanceJoint2D();
					var joints = pointA.GetComponentsInChildren<SpringJoint>();	//get all DistanceJoint2D component from pointA
					SpringJoint	distJoint = new SpringJoint();
...
//						distJoint = pointB.gameObject.AddComponent<DistanceJoint2D>();
						distJoint = pointB.gameObject.AddComponent<SpringJoint>();
...
//				GameObject.DestroyImmediate (chain.GetComponent<HingeJoint2D>());	//remove HingeJoint2D component for last chain
				GameObject.DestroyImmediate (chain.GetComponent<HingeJoint>());	//remove HingeJoint2D component for last chain
...
//			var hingeHolder = objHolder.GetComponentsInChildren<HingeJoint2D>();
			var hingeHolder = objHolder.GetComponentsInChildren<HingeJoint>();
