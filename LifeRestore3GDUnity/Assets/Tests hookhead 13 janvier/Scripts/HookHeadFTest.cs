...
public class HookHeadFTest : MonoBehaviour {
	...
	void Start(){
		...
		if(_myShooter.GetComponent<ShootFTest>().playerIndex==XInputDotNetPure.PlayerIndex.One){
			gameObject.transform.Find("B 5JanvTest").renderer.material.color=Color.yellow;
			gameObject.renderer.material.color=Color.yellow;
		}
		if(_myShooter.GetComponent<ShootFTest>().playerIndex==XInputDotNetPure.PlayerIndex.Two){
			gameObject.transform.Find("B 5JanvTest").renderer.material.color=Color.red;
			gameObject.renderer.material.color=Color.red;
		}		
		if(_myShooter.GetComponent<ShootFTest>().playerIndex==XInputDotNetPure.PlayerIndex.Three){
//			gameObject.transform.Find("B 5Janv").renderer.material.color=Color.green;
			gameObject.transform.Find("B 5JanvTest").renderer.material.color=new Color32(107,142,35,255);
			//			gameObject.renderer.material.color=Color.green;
			gameObject.renderer.material.color=new Color32(107,142,35,255);

		}		
		if(_myShooter.GetComponent<ShootFTest>().playerIndex==XInputDotNetPure.PlayerIndex.Four){
			gameObject.transform.Find("B 5JanvTest").renderer.material.color=Color.blue;
	...
	void OnTriggerEnter(Collider _Collided){
		...
		//ni un lien
		if(_Collided.gameObject != _myShooter && _Collided.gameObject!= _myShooter.transform.Find("ApparenceAvatar").gameObject && _Collided.gameObject.name!="NewHookheadTest(Clone)" && _Collided.gameObject.name!="B 5JanvTest"){
...
//				}
				...
				if(howWasIShot==1)_myShooter.GetComponent<ShootFTest>()._target=_Collided.gameObject;
				if(howWasIShot==2)_myShooter.GetComponent<ShootFTest>()._target1=_Collided.gameObject;
...
//				Debug.Log("contact?");
				if(howWasIShot==1) Destroy(_myShooter.GetComponent<ShootFTest>()._myHook);
				if(howWasIShot==2) Destroy(_myShooter.GetComponent<ShootFTest>()._myHook1);
			}
		}
	}
...
//			if(_diffPosition!=null){
			...
			if(Vector3.Distance(gameObject.transform.position, _myShooterPos)>=v_BreakDistance){
				_myShooter.GetComponent<ShootFTest>().DetachLink(howWasIShot - 1);
			}
		}
	}
	...
	void FixedUpdate (){
		if(_myShooter!=null){
			...
			if (GrappedTo == null) {
				...
				if(shouldIReturn==true){
					if(Vector3.Distance(gameObject.transform.position, gameObject.transform.Find("B 5JanvTest").GetComponent<InTheMiddle5JanvTest>()._whereIsItShot )<=v_allowedProximity){
						Debug.Log("HEADSHOT!");
						if(howWasIShot==1) Destroy(_myShooter.GetComponent<ShootFTest>()._myHook);
						if(howWasIShot==2) Destroy(_myShooter.GetComponent<ShootFTest>()._myHook1);
					}
					...
					gameObject.rigidbody.AddForce(whereShouldIGo*_myShooter.GetComponent<ShootFTest>().v_SpeedBullet*v_returnSpeedConst);
				}
			}
		}
	}
