Shader "Hidden/SSAO Pro V1"
{
	...
	CGINCLUDE
		...
		inline float3 getVSPosition(float2 uv)
		{
			...
			float depth = LinearEyeDepth(tex2D(_DepthNormalMapF32, uv).x);
		...
		{
			float3 diff = getVSPosition(tcoord + uv) - p; // View space
